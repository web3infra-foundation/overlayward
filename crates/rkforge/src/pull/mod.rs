...
fn sync_pull_or_get_image_with_policy_and_output_with_tls(
    ...
    let do_pull = async move {
        ...
        let (image_manifest, final_digest) =
            resolve_image_manifest(&client, &image_ref, &auth_method, manifest, digest).await?;
        let layers = pull_layers(&client, &image_ref, &image_manifest, no_cache, quiet).await?;
        let manifest = OciManifest::Image(image_manifest);
        let manifest_path = write_manifest(&image_ref, &manifest, &final_digest).await?;
...
/// Resolve an `OciManifest` to a concrete `OciImageManifest`, following image indexes
/// to the platform-specific entry for the current OS/arch.
        ...
        OciManifest::ImageIndex(idx) => {
            ...
            let entry = idx
                ...
                .find(|e| {
                    e.platform
                        .as_ref()
                        .map(|p| p.architecture == current_arch && p.os == "linux")
                        .unwrap_or(false)
                })
                .ok_or_else(|| anyhow!("No manifest for linux/{current_arch} in image index"))?;

            let platform_ref = image_ref.clone_with_digest(entry.digest.clone());
            let (platform_manifest, platform_digest) = client
                .pull_manifest(&platform_ref, auth_method)
                .await
                .with_context(|| format!("Failed to pull platform manifest {}", entry.digest))?;
            match platform_manifest {
                OciManifest::Image(m) => Ok((m, platform_digest)),
                OciManifest::ImageIndex(_) => anyhow::bail!("Nested image indexes not supported"),
            }
        }
    }
}
...
async fn pull_or_get_image_with_policy_and_tls(
    ...
    let layers = pull_layers(&client, &image_ref, &image_manifest, no_cache, false).await?;
...
pub fn sync_pull_or_get_image_with_config(
    ...
    let do_pull = async move {
        ...
        let (image_manifest, final_digest) =
            resolve_image_manifest(&client, &image_ref, &auth_method, manifest, digest).await?;
        let layers = pull_layers(&client, &image_ref, &image_manifest, false, false).await?;
        let manifest = OciManifest::Image(image_manifest);
        let manifest_path = write_manifest(&image_ref, &manifest, &final_digest).await?;
        Ok((manifest_path, layers))
    };
