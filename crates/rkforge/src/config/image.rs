...
/// Initialize the global CONFIG. Must be called before any pull/storage/images operation.
/// Repeated calls with identical fields are safely ignored.
/// Repeated calls with different fields panic (split-brain detection).
pub fn init_config(config: Config) {
    if let Err(new_config) = CONFIG.set(config) {
        let existing = get_config();
        assert_eq!(
            existing.layers_store_root, new_config.layers_store_root,
            "rkforge CONFIG split-brain: layers_store_root mismatch: {:?} vs {:?}",
            existing.layers_store_root, new_config.layers_store_root,
        );
        assert_eq!(
            existing.default_registry, new_config.default_registry,
            "rkforge CONFIG split-brain: default_registry mismatch: {:?} vs {:?}",
            existing.default_registry, new_config.default_registry,
        );
        assert_eq!(
            existing.is_root, new_config.is_root,
            "rkforge CONFIG split-brain: is_root mismatch: {:?} vs {:?}",
            existing.is_root, new_config.is_root,
        );
        assert_eq!(
            existing.build_dir, new_config.build_dir,
            "rkforge CONFIG split-brain: build_dir mismatch: {:?} vs {:?}",
            existing.build_dir, new_config.build_dir,
        );
        assert_eq!(
            existing.metadata_dir, new_config.metadata_dir,
            "rkforge CONFIG split-brain: metadata_dir mismatch: {:?} vs {:?}",
            existing.metadata_dir, new_config.metadata_dir,
        );
    }
}

/// Get the initialized CONFIG. Panics if not yet initialized.
pub fn get_config() -> &'static Config {
    CONFIG.get().expect("rkforge CONFIG not initialized — call init_config() first")
}

static REGISTRY: &str = "registry-1.docker.io";
static ROOT_PATH: &str = "/var/lib/overlayward";
...
/// Configuration for rkforge storage and registry.
...
mod tests {
    ...
    fn test_default_storage_root_for_root() {
        ...
        assert_eq!(path.to_string_lossy(), "/var/lib/overlayward");
    }
